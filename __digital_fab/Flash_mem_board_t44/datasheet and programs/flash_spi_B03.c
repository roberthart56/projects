...
void read_address(char *byte_ret, char *address_array){
	unsigned char ind;
	clear(CS_port,CS_pin);
	SPI_trade(READ_DATA, byte_ret);
	
	for (ind = 0; ind < 3; ++ind) {
		SPI_trade(address_array[ind], byte_ret);
	}
...
int main(void) {
	...
	while (1) {
		...
		read_address(&byte_received,read_addr);		//read addressk sending single byte pointer
