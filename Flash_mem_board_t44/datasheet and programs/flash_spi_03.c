...
//    }
   ...
   void SPI_read_status(unsigned char *ret) {
      ...
      *ret = 0;
      ...
      void SPI_read(unsigned char *ret) {
         unsigned char bit,ind;
         unsigned char cmd = READ_DATA;
		 unsigned char add[4]={cmd,0,0,2};
   	  //
         // init
         //
         *ret = 0;
...
//
    ...
    void SPI_write() {
		...
		unsigned char add[5]={cmd,0,0,2, 0XAB};
		  	  //
		        // init
		        //
		clear(MOSI_port,MOSI_pin);
...
int main(void) {
   ...
   //
   while (1) {
	  SPI_write_enable();  //write enable
	  SPI_read_status(&ret);
	  put_char(&serial_port,serial_pin_out,ret);
	  char_delay();
	  SPI_write();	//write 0XAA to memory location 0.
	  SPI_delay();  // delay for no apparent reason.
	  SPI_read(&ret);   //read from memory location 0.
      put_char(&serial_port,serial_pin_out,ret);
