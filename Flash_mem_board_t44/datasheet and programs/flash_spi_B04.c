...
void read_status_register_1 (char *byte_ret){
	clear(CS_port,CS_pin);
	SPI_trade(READ_ST_REG_1, byte_ret);
	SPI_trade(0x00, byte_ret);  //send function the pointer to the variable.
	set(CS_port,CS_pin);
}//

void read_address(char *out_array, char *address_array){
	unsigned char ind;
...
int main(void) {
	char byte_received;
	char received_array[2];
	char dummy;
	char read_addr[3] = {0,0,0};
	...
	SPI_trade(0x00, &dummy);
	SPI_trade(0x00, &dummy);
	SPI_trade(0x00, &dummy);		//addr3
	set(CS_port,CS_pin);
	...
	SPI_trade(0x00, &dummy);
	SPI_trade(0x00, &dummy);		//addr3
	SPI_trade(0x22, &dummy);		//data written
	SPI_trade(0x23, &dummy);	
	set(CS_port,CS_pin);
	SPI_delay();
	...
	while (1) {
		...
		read_address(received_array,read_addr);		//read addressk sending single byte pointer
		put_char(&serial_port,serial_pin_out,received_array[0]);
		
		
		put_char(&serial_port,serial_pin_out,received_array[1]);
