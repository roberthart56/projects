...
void read_address(char *byte_ret){
	clear(CS_port,CS_pin);
	SPI_trade(READ_DATA, byte_ret);
	SPI_trade(0x00, byte_ret);
	SPI_trade(0x00, byte_ret);
	SPI_trade(0x00, byte_ret);		//addr3
	SPI_trade(0x00, byte_ret);		//data read
	set(CS_port,CS_pin);	
}
// main
//
int main(void) {
	...
	SPI_trade(0x00, &dummy);
	SPI_trade(0x00, &dummy);		//addr3
	SPI_trade(0x23, &byte_received);		//data written
	set(CS_port,CS_pin);
	SPI_delay();
	...
	while (1) {
		...
		read_address(&byte_received);		//read addressk sending single byte pointer
		put_char(&serial_port,serial_pin_out,byte_received);
		_delay_ms(500);
