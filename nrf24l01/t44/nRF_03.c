// nRF_01.c  12/30/16  Modified from NG Hello-world below:
// Add functin to trade one byte acc. to SPI protocol.
//This will be the basis of all other functions.
//Add function for SPI trade? that has value unsigned char  nRF_03.c
//
//
...
//
   void SPI_read_status(unsigned char *ret) {
      ...
      *ret = 0;
	  ...
	  //
	  //
      unsigned char SPI_trans(unsigned char command) {
      unsigned char bit;
      unsigned char ret;//
      // init
      //
   ...
   void SPI_exchange(unsigned char command,/*unsigned char bytes,unsigned char *write_buffer,*/unsigned char *read_buffer) {
   unsigned char bit;
   //
   // init
   //
...
int main(void) {
   ...
   while (1) {
	   set(CS_port,CS_pin);
	   //result = SPI_trans(0x00);
	   //result = SPI_trans(NOP);
	   
	   SPI_exchange(0x00, &result);
	   SPI_exchange(NOP, &result);
