...
def pos(x):
     return(int(Num_pix*x/max_adc))

def color(x,y,z):
    r = min(255, int(abs(x)*255/max_adc))
    g = min(255, int(abs(y)*255/max_adc))
    b = min(255, int(abs(z)*255/max_adc))
    return(r,g,b)
...
while True:
    ...
    (r,g,b) = color(d2,d3,d4)
    
    integers = (pos(d1),r,g,b)
    print(integers)
