...
//    }
   ...
   void SPI_read_status(unsigned char *ret) {
      ...
      *ret = 0;
      ...
      void SPI_read(char *ret, unsigned char nbytes, char *address_array) {
         ...
         *ret = 0;
         ...
         SPI_delay();
         //
         // bit loop to send commnand and address bytes
         //
	for (ind = 0; ind < 4; ++ind) {
	   	for (bit = 0; bit < 8; ++bit) {
	   		  //if bit_test(cmd, (7 - bit)) set(MOSI_port, MOSI_pin);
 	...
 	clear(MOSI_port,MOSI_pin);  //this should not matter.
   	  //
         // bit loop to read nbytes of bytes
         //
...
//
    ...
    void SPI_command(unsigned char cmd, char *address_array, unsigned char nbytes, char *write_buffer) {		//make array of command, address, data
		unsigned char bit,ind;
		...
		send[4] = write_buffer[0];
		send[5] = write_buffer[1];
...
int main(void) {
   ...
   char write_data[2] = {22,10};//
   char write_addr[3] = {0,0,22};
   char read_addr[3] = {0,0,22};
   char read_data[2];
   ...
   SPI_command(WRITE_DATA, write_addr, 2, write_data);	
   // main loop
   //
   while (1) {
	  
	  SPI_read(read_data,2,read_addr);   //read from memory location 0.
      put_char(&serial_port,serial_pin_out,read_data[0]);
      put_char(&serial_port,serial_pin_out,read_data[1]);
      char_delay();
      //
      // to be written
      //
